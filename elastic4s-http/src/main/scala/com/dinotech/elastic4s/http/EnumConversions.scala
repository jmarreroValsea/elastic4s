...
import com.dinotech.elastic4s.{DistanceUnit, VersionType}
import com.dinotech.elastic4s.DistanceUnit.{Centimeters, Feet, Inch, Kilometers, Meters, Miles, Millimeters, NauticalMiles, Yard}
import com.dinotech.elastic4s.cluster.ClusterHealthLevel
import com.dinotech.elastic4s.json.{XContentBuilder, XContentFactory}
import com.dinotech.elastic4s.searches.QueryRescoreMode.{Avg, Max, Min, Multiply, Total}
import com.dinotech.elastic4s.searches.aggs.{HistogramOrder, SubAggCollectionMode, TermsOrder}
import com.dinotech.elastic4s.searches.queries.funcscorer.{CombineFunction, FunctionScoreQueryScoreMode, MultiValueMode}
import com.dinotech.elastic4s.searches.queries.geo.GeoDistance.{Arc, Plane}
import com.dinotech.elastic4s.searches.queries.geo.{GeoDistance, GeoExecType, GeoValidationMethod}
import com.dinotech.elastic4s.searches.queries.matches.MultiMatchQueryBuilderType.{BEST_FIELDS, CROSS_FIELDS, MOST_FIELDS, PHRASE, PHRASE_PREFIX}
import com.dinotech.elastic4s.searches.queries.matches.{MultiMatchQueryBuilderType, ZeroTermsQuery}
import com.dinotech.elastic4s.searches.queries.{RegexpFlag, SimpleQueryStringFlag}
import com.dinotech.elastic4s.searches.sort.{SortMode, SortOrder}
import com.dinotech.elastic4s.searches.suggestion.{Fuzziness, SortBy, StringDistanceImpl, SuggestMode}
import com.dinotech.elastic4s.searches.{DateHistogramInterval, QueryRescoreMode, ScoreMode}
