...
import com.dinotech.elastic4s.http.search.RawQueryBodyFn
import com.dinotech.elastic4s.http.search.queries.compound.{
  BoolQueryBuilderFn,
  BoostingQueryBodyFn,
  ConstantScoreBodyFn,
  DisMaxQueryBodyFn
}
import com.dinotech.elastic4s.http.search.queries.geo.{
  GeoBoundingBoxQueryBodyFn,
  GeoDistanceQueryBodyFn,
  GeoPolyonQueryBodyFn,
  GeoShapeQueryBodyFn
}
import com.dinotech.elastic4s.http.search.queries.nested.{
  HasChildBodyFn,
  HasParentBodyFn,
  NestedQueryBodyFn,
  ParentIdQueryBodyFn
}
import com.dinotech.elastic4s.http.search.queries.span._
import com.dinotech.elastic4s.http.search.queries.specialized.{
  FunctionScoreQueryBuilderFn,
  MoreLikeThisBuilderFn,
  ScriptQueryBodyFn,
  ScriptScoreQueryBodyFn
}
import com.dinotech.elastic4s.http.search.queries.term._
import com.dinotech.elastic4s.http.search.queries.text._
import com.dinotech.elastic4s.json.{XContentBuilder, XContentFactory}
import com.dinotech.elastic4s.searches.queries.funcscorer.{FunctionScoreQuery, ScriptScore}
import com.dinotech.elastic4s.searches.queries.geo.{
  GeoBoundingBoxQuery,
  GeoDistanceQuery,
  GeoPolygonQuery,
  GeoShapeQuery
}
import com.dinotech.elastic4s.searches.queries.matches._
import com.dinotech.elastic4s.searches.queries.span._
import com.dinotech.elastic4s.searches.queries.term.{TermQuery, TermsLookupQuery, TermsQuery, TermsSetQuery}
import com.dinotech.elastic4s.searches.queries.{IdQuery, _}
