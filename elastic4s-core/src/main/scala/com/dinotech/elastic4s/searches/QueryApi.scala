...
import com.dinotech.elastic4s.searches.queries._
import com.dinotech.elastic4s.searches.queries.funcscorer.FunctionScoreQuery
import com.dinotech.elastic4s.searches.queries.geo._
import com.dinotech.elastic4s.searches.queries.matches._
import com.dinotech.elastic4s.searches.queries.span._
import com.dinotech.elastic4s.searches.queries.term._
import com.dinotech.elastic4s.{DistanceUnit, DocumentRef, Indexable}
...
trait QueryApi {
  ...
  def termsSetQuery(field: String,
      terms: Set[String],
      minimumShouldMatchField: String): com.dinotech.elastic4s.searches.queries.term.TermsSetQuery =
    com.dinotech.elastic4s.searches.queries.term.TermsSetQuery(field, terms, minimumShouldMatchField=Some(minimumShouldMatchField))

  def termsSetQuery(field: String,
      terms: Set[String],
      minimumShouldMatchScript: Script): com.dinotech.elastic4s.searches.queries.term.TermsSetQuery =
    com.dinotech.elastic4s.searches.queries.term.TermsSetQuery(field, terms, minimumShouldMatchScript=Some(minimumShouldMatchScript))
